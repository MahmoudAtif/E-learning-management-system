...
class Category(models.Model):
    ...
    icon=models.CharField(max_length=100 , null=True , blank=True , default='fa-solid fa-ghost')
...
class Author(models.Model):
    ...
    def get_total_rating(self):
        total=CommentCourse.objects.filter(course__author=self).aggregate(average=Avg('rate'),sum=Sum('rate'))
        return total
...
class Course(models.Model):
    ...
    author = models.ForeignKey(Author,on_delete=models.CASCADE , related_name='author_courses')
    ...
    certificate=models.CharField(null=True ,choices=CERTIFICATE, default='No', max_length=50)
    ...
    @property
    def get_rating(self):
        rating=CommentCourse.objects.filter(course=self).aggregate(average=Avg('rate'))['average']
        return rating
...
class Requirment(models.Model):
    course=models.ForeignKey(Course , on_delete=models.CASCADE, related_name='course_requirments')
    point=models.CharField(max_length=100)
    date_created = models.DateField(auto_now_add=True)
    date_updated = models.DateField(auto_now=True)
...
class ShopCart(models.Model):
    student=models.ForeignKey(Student, on_delete=models.CASCADE , related_name='student_carts')
...
class Checkout(models.Model):
    student=models.ForeignKey(Student, on_delete=models.CASCADE,related_name='student_checkout')
    price=models.DecimalField(max_digits=5, decimal_places=2)    
...
class CheckoutItem(models.Model):
    student=models.ForeignKey(Student, on_delete=models.CASCADE,related_name='checkoutItems')
...
class CommentVideo(models.Model):
    student=models.ForeignKey(Student, on_delete=models.CASCADE , related_name='student_video_comments')
