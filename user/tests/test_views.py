...
class TestView(TestCase):
    ...
    def test_login_page_func(self):
        response=self.client.get(self.login_page_url,HTTP_ACCEPT='application/json')
        self.assertEquals(response.status_code,200)
        self.assertTemplateUsed(response,'login.html')
        print('test_login_page_func --- DONE')

    def test_login_func(self):
        response=self.client.post(self.login_url,{'username':'admin','password':'admin'},follow=True) #follow --> the client will follow any redirects
        self.assertEquals(response.status_code,200)
